...
class UserManager: ObservableObject {
    ...
    func updateUserPoints( points: Int) {
        ...
        let newRank = UserRank.getRank(for: points)
   ...
   func addPaymentHistory(history: PaymentHistory ) {
    ...
    func changeSupportPlayer(from oldPlayerId: String?, to newPlayerId: String) {
        ...
        if safeOldId == newPlayerId {
            print("ℹ️ Already supporting this player (Argument Check).") // ℹ️ すでにこの選手を応援しています
            return
        }
        
        if self.selectedPlayerID == newPlayerId {
            print("ℹ️ Already supporting this player (State Check).") // ℹ️ すでにこの選手を応援しています
            return
        }
        ...
        db.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            if let error = error {
                ...
                print("✅ Player change successful: \(safeOldId ?? "None") -> \(newPlayerId)") // ✅ 選手変更成功
